...
{
  for (unsigned i = 0; i < numMods; i++)
    ums[i]->handleOverlayDraw();
}
void UsermodManager::appendConfigData(Print &dest)
{
  for (unsigned i = 0; i < numMods; i++)
    ums[i]->appendConfigData(dest);
}
...
bool UsermodManager::getUMData(um_data_t **data, uint8_t mod_id)
{
  for (unsigned i = 0; i < numMods; i++)
  {
    if (mod_id > 0 && ums[i]->getId() != mod_id)
      continue; // only get data form requested usermod if provided
    if (ums[i]->getUMData(data))
      return true; // if usermod does provide data return immediately (only one usermod can provide data at one time)
  }
  return false;
}
...
{
  for (unsigned i = 0; i < numMods; i++)
    ums[i]->onMqttConnect(sessionPresent);
}
bool UsermodManager::onMqttMessage(char *topic, char *payload)
{
  for (unsigned i = 0; i < numMods; i++)
    if (ums[i]->onMqttMessage(topic, payload))
      return true;
  return false;
}
#endif
...
{
  for (unsigned i = 0; i < numMods; i++)
    if (ums[i]->onEspNowMessage(sender, payload, len))
      return true;
  return false;
}
#endif
...
{
  if (numMods >= WLED_MAX_USERMODS || um == nullptr)
    return false;
  ums[numMods++] = um;
  return true;
}
