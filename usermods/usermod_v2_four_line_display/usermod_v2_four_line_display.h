...
public:
  ...
  void loop()
  {
    if (!enabled || millis() - lastUpdate < (clockMode ? 1000 : refreshRate) || strip.isUpdating())
      return;
    lastUpdate = millis();
  ...
  void drawLine(uint8_t line, Line4Type lineType)
  {
    ...
    case FLD_LINE_MODE:
      ...
      for (; printedChars < getCols() - 2 && printedChars < LINE_BUFFER_SIZE - 3; printedChars++)
  ...
  void showTime(bool fullScreen = true)
  {
    ...
    if (fullScreen)
    {
      ...
      if (useAMPM)
        drawString(12 + (fullScreen ? 0 : 2), lineHeight * 2, (isAM ? "AM" : "PM"), true);
      else
    ...
    else
    {
      ...
      if (useAMPM)
        drawString(12 + (fullScreen ? 0 : 2), lineHeight * 3, (isAM ? "AM" : "PM"), true);
    }
  }
  ...
  bool readFromConfig(JsonObject &root)
  {
    ...
    else
    {
      ...
      if (pinsChanged || type != newType)
      {
        ...
        if (isSPI)
        {
          if (ioPin[0] == spi_sclk && ioPin[1] == spi_mosi)
            po = PinOwner::HW_SPI; // allow multiple allocations of HW SPI bus pins
          pinManager.deallocateMultiplePins((const uint8_t *)ioPin, 5, po);
        }
        else
        {
          if (ioPin[0] == i2c_scl && ioPin[1] == i2c_sda)
            po = PinOwner::HW_I2C; // allow multiple allocations of HW I2C bus pins
